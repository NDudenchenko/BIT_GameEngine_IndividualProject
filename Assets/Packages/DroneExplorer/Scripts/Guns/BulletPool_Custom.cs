...
namespace DroneExplorer
{
    ...
    public class BulletPool_Custom : MonoBehaviour
    {
        public static BulletPool_Custom instance;  // public static reference to self
        public int count;                   // maximum bullets in scene
        public GameObject baseBullet;       // link to bullet prefab
        Bullet_Custom[] myBullets;                 // internal storage of bullets
        ...
        void Start()
        {
            ...
            myBullets = new Bullet_Custom[count];
            for (int itor = 0; itor < count; itor++)
            {
                GameObject temp = Instantiate(baseBullet, transform);
                myBullets[itor] = temp.GetComponent<Bullet_Custom>();
